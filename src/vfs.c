...
//EDIT 
void add_write_info(const char* path,size_t size,off_t offset)
{
    ...
    if(w_found == NULL){
    	...
    	if(w_tmp2 == NULL)
    	    log_msg("HIMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM\n");
    }  
    else
    {
	     ...
	     struct fuse_file_info *fi)
{
    ...
    log_msg("Goiing..........................\n");
    add_write_info(path,size,offset);
    return retstat;
}
...
/**
  ...
  */
void remove_write_info(const char *path)
{
    WriteInfo *w_tmp = (WriteInfo *)malloc(sizeof(WriteInfo));
    strcpy(w_tmp->full_path, path);
...
//EDIT
//Versioning
void version_file(const char *path)
{
    //TODO
    //Call the versioning function here
    
    remove_write_info(path);
}
